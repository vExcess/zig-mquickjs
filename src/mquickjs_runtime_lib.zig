            ...
            .function_call => {
                ...
                if (p == null or mc.objectClassId(p.?) == c.JS_CLASS_C_FUNCTION) {
                    ...
                    switch (fd.def_type) {
                        ...
                        c.JS_CFUNC_f_f => {
                            var d: f64 = undefined;
                            if (JS_ToNumber(ctx, &d, rt.slot(fp, rt.FRAME_OFFSET_ARG0).*) != 0) {
                                val = c.JS_EXCEPTION;
                            } else {
                ...
                switch (opcode) {
                    ...
                    OP.lt, OP.lte, OP.gt, OP.gte, OP.eq, OP.neq, OP.strict_eq, OP.strict_neq => {
                        ...
                        if (rt.isBothInt(op1, op2)) {
                            ...
                            saveFrame(ctx, fp, sp, pc, b.?);
                            val = switch (opcode) {
                                OP.lt, OP.lte, OP.gt, OP.gte => js_relational_slow(ctx, opcode),
                                OP.eq => js_eq_slow(ctx, 0),
                                OP.neq => js_eq_slow(ctx, 1),
                                OP.strict_eq => js_strict_eq_slow(ctx, 0),
                                else => js_strict_eq_slow(ctx, 1),
                            };
                            const restored = restorePc(fp);
                            b = restored.b;
                            pc = restored.pc;
                    ...
                    OP.for_in_start, OP.for_of_start => {
                        saveFrame(ctx, fp, sp, pc, b.?);
                        val = js_for_of_start(ctx, @intFromBool(opcode == OP.for_in_start));
                        const restored = restorePc(fp);
                        b = restored.b;
                        pc = restored.pc;
                        if (vt.isExactException(val)) {
